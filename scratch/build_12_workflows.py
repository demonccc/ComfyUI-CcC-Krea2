...
def base_loaders():
    return [
        {
            "id": 1, "type": "UNETLoader", "pos": [100, 100], "size": [315, 82], "flags": {}, "order": 0, "mode": 0,
            "inputs": [], "outputs": [{"name": "MODEL", "type": "MODEL", "links": [1], "slot_index": 0}],
            "properties": {"Node name for S&R": "UNETLoader"}, "widgets_values": ["krea2_unet.safetensors", "default"]
        },
        {
            "id": 2, "type": "CLIPLoader", "pos": [100, 220], "size": [315, 82], "flags": {}, "order": 1, "mode": 0,
            "inputs": [], "outputs": [{"name": "CLIP", "type": "CLIP", "links": [2], "slot_index": 0}],
            "properties": {"Node name for S&R": "CLIPLoader"}, "widgets_values": ["qwen2_5_vl.safetensors", "qwen2_5_vl", "default"]
        },
        {
            "id": 3, "type": "VAELoader", "pos": [100, 340], "size": [315, 82], "flags": {}, "order": 2, "mode": 0,
            "inputs": [], "outputs": [{"name": "VAE", "type": "VAE", "links": [3], "slot_index": 0}],
            "properties": {"Node name for S&R": "VAELoader"}, "widgets_values": ["krea2_vae.safetensors"]
        }
    ]
...
def build_modular_edit_pipeline(
    ...
    for idx, (role_name, role_node_type, aliases, directive, extra_widgets) in enumerate(role_specs):
        ...
        prep_node = {
            ...
            "pos": [460, role_pos_y],
            "size": [315, 220],
            "flags": {}, "order": 5 + idx * 3, "mode": 0,
            "inputs": [
                {"name": "clip", "type": "CLIP", "link": 2},
                {"name": "image", "type": "IMAGE", "link": img_out_link}
            ],
            "outputs": [
                {"name": "prepared_image", "type": "CCC_KREA2_PREPARED_IMAGE", "links": [prep_out_link], "slot_index": 0},
    ...
    # Base loader links
    links.extend([
        [1, 1, 0, edit_id, 0, "MODEL"],
        [2, 2, 0, edit_id, 1, "CLIP"],
        [3, 3, 0, target_latent_id, 0, "VAE"]
    ])
    ...
    links.extend([
        [ksamp_out_link, ksampler_id, 0, decode_id, 0, "LATENT"],
        [vae_decode_link, 3, 0, decode_id, 1, "VAE"]
    ])
...
def wf_01():
    nodes = base_loaders() + [
        {
            "id": 4, "type": "CcCKrea2TextToImage", "pos": [460, 100], "size": [400, 260], "flags": {}, "order": 3, "mode": 0,
            "inputs": [
                {"name": "model", "type": "MODEL", "link": 1},
                {"name": "clip", "type": "CLIP", "link": 2},
                {"name": "prompt_augmentation", "type": "CCC_KREA2_PROMPT_AUGMENTATION", "link": None}
        ...
        {
            "id": 5, "type": "KSampler", "pos": [900, 100], "size": [315, 262], "flags": {}, "order": 4, "mode": 0,
            "inputs": [
        ...
        {
            "id": 6, "type": "VAEDecode", "pos": [1250, 100], "size": [210, 46], "flags": {}, "order": 5, "mode": 0,
            "inputs": [
        ...
        {
            "id": 7, "type": "SaveImage", "pos": [1500, 100], "size": [315, 270], "flags": {}, "order": 6, "mode": 0,
            "inputs": [{"name": "images", "type": "IMAGE", "link": 9}],
            "outputs": [],
            "properties": {"Node name for S&R": "SaveImage"}, "widgets_values": ["Krea2_T2I"]
        }
    ]
    links = [
        [1, 1, 0, 4, 0, "MODEL"],
...
def wf_02():
    nodes = base_loaders() + [
        {
            "id": 4, "type": "CcCKrea2LoRAPromptSettings", "pos": [460, 400], "size": [315, 300], "flags": {}, "order": 3, "mode": 0,
            "inputs": [],
            "outputs": [{"name": "lora_prompt_settings", "type": "CCC_KREA2_LORA_PROMPT_SETTINGS", "links": [4], "slot_index": 0}],
            "properties": {"Node name for S&R": "CcCKrea2LoRAPromptSettings"},
            "widgets_values": [True, True, "style", " cinematic lighting", "", False, "outfit", "", "", False, "pose", "", "", False, "identity", "", ""]
        },
        {
            "id": 5, "type": "CcCKrea2LoRAStack", "pos": [460, 100], "size": [315, 260], "flags": {}, "order": 4, "mode": 0,
            "inputs": [
                {"name": "model", "type": "MODEL", "link": 1},
            ...
            "widgets_values": [True, 1.0, True, "krea2_style_lora.safetensors", 0.8, False, "", 1.0, False, "", 1.0, False, "", 1.0]
        },
        {
            "id": 6, "type": "CcCKrea2TextToImage", "pos": [820, 100], "size": [400, 260], "flags": {}, "order": 5, "mode": 0,
            "inputs": [
        ...
        {
            "id": 7, "type": "KSampler", "pos": [1260, 100], "size": [315, 262], "flags": {}, "order": 6, "mode": 0,
            "inputs": [
        ...
        {
            "id": 8, "type": "VAEDecode", "pos": [1610, 100], "size": [210, 46], "flags": {}, "order": 7, "mode": 0,
            "inputs": [
        ...
        {
            "id": 9, "type": "SaveImage", "pos": [1850, 100], "size": [315, 270], "flags": {}, "order": 8, "mode": 0,
            "inputs": [{"name": "images", "type": "IMAGE", "link": 12}],
            "outputs": [],
            "properties": {"Node name for S&R": "SaveImage"}, "widgets_values": ["Krea2_T2I_LoRA"]
        }
    ]
    links = [
        [1, 1, 0, 5, 0, "MODEL"],
...
def write_all_12_workflows():
    ...
    wf07 = build_modular_edit_pipeline([
    ...
    wf08 = build_modular_edit_pipeline([
        ("subject", "CcCKrea2SubjectImage", "subject_image, Image {slot}", "", [1.0, 0.0, 0.0, 1.0, 1.0, "auto"])
    ], target_content="subject", inpaint_mask=True)
    ...
    wf11 = build_modular_edit_pipeline([
        ("subject", "CcCKrea2SubjectImage", "subject_image", "Apply high contrast lighting.", [1.5, 0.2, 0.1, 1.2, 1.0, "crop"])
    ])
