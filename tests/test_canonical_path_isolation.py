...
def test_legacy_functions_not_called_in_canonical_path():
    ...
    (chain,) = ref_node.process(
        reference_path="edit", prepared_image=prep, alias="subject", vision_instruction="Use subject identity"
    )
    ...
    with (
        patch.object(
            conditioning,
            "build_krea2_qwen_template",
            side_effect=RuntimeError("Legacy build_krea2_qwen_template called!"),
        ),
        patch.object(
            conditioning,
            "build_annotated_user_prompt",
            side_effect=RuntimeError("Legacy build_annotated_user_prompt called!"),
        ),
    ):
        ...
        easy_out = easy_node.process(
            model=fresh_model, clip=mock_clip, vae=mock_vae, positive_prompt="a photo", preset="balanced", subject=img
        )
...
def test_physical_image_count_matches_vision_markers():
    ...
    (chain1,) = ref_node.process(
        reference_path="edit", prepared_image=prep, alias="subject", vision_instruction="Subject identity"
    )
    (chain2,) = ref_node.process(
        reference_path="style", prepared_image=prep, style_processing="2x2", previous_references=chain1
    )
