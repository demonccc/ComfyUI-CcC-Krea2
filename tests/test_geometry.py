...
def test_reference_fit_transform_modes():
    ...
    # Fit mode
    fit_img, fit_mask = apply_reference_fit_transform(img, target_h=1024, target_w=1024, mode="fit", mask=mask)
    assert fit_img.shape == (1, 1024, 1024, 3)

    # Crop mode
    crop_img, crop_mask = apply_reference_fit_transform(img, target_h=1024, target_w=1024, mode="crop", mask=mask)
    assert crop_img.shape == (1, 1024, 1024, 3)
