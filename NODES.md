...
- `subject_transfer_1` / `subject_transfer_2`: BFS-aligned `body_swap:` prompt that transfers the complete Subject with its clothing and accessories, while matching the Scene character's exact position, pose, action, role, and interactions. Scene is an appearance reference at `1.0`; its automatic Style path is indirect and uses a semantic guardrail that preserves composition, pose, actions, interactions, objects, and visual treatment while excluding the replaced person's identity, anatomy, clothing, and accessories.
- `flexible_subject_transfer_1` / `flexible_subject_transfer_2`: canonical flexible complete-subject replacement prompt.
