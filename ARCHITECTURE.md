...
1. **Easy Routing Engine**: Subject, Scene, Outfit, and Style sources are managed via a policy-based capability matrix (`outfit_policy`: `user` | `disabled`; `style_policy`: `user` | `scene_auto` | `disabled`). The unified routing matrix dynamically resolves visual context based on the selected preset (`flexible`, `balanced`, `consistent`, `preserve_identity`, `max_identity`, `subject_transfer`, `preserve_scene`, `outfit_transfer`, `style_transfer`, `scene_reinterpretation`) and connected inputs.
   ...
   - *Subject + Scene Multi-Reference Routing*: When both Subject and Scene are connected, the free-target family keeps output geometry Scene-driven while target content remains empty. The identity-anchored family shifts both target content and geometry to the Subject image. `preserve_scene` explicitly anchors both target content and geometry to the Scene image. `scene_reinterpretation` uses the Scene for geometry and instruction guidance without initializing target latent from Scene pixels. `subject_transfer` uses Scene for geometry and target latent initialization while driving strong Subject transfer (boost: 8.0).
   - *Automatic Style Policy (`scene_auto`)*: Applied to `subject_transfer` and `scene_reinterpretation`. Automatically assigns the Scene reference as the style source when Scene is connected.
