...
1. **Easy Routing Engine**: Subject, Scene, and Outfit logical roles are generated by the preset recipe. The unified routing matrix dynamically resolves the visual context based on the selected preset (e.g., `flexible`, `balanced`, `consistent`, `preserve_identity`, `max_identity`, `subject_transfer`, `preserve_scene`, `outfit_transfer`, `style_transfer`) and the connected image inputs. This matrix automatically builds the internal reference chain, dynamically constructs semantic Scene+Outfit instructions, and filters disconnected references.
   - *Free Target Content Family* (`flexible`, `balanced`, `consistent`): Subject-only `target_content = empty`. Denoising starts from noise, favoring edit flexibility.
   ...
   - *Subject + Scene Multi-Reference Routing*: When both Subject and Scene are connected, the free-target family keeps output geometry Scene-driven while target content remains empty. The identity-anchored family shifts both target content and geometry to the Subject image. `preserve_scene` explicitly anchors both target content and geometry to the Scene image.
