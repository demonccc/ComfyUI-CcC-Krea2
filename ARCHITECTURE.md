...
1. **Easy Routing Engine**: Subject, Scene, Outfit, and Style sources are managed via a policy-based capability matrix (`outfit_policy`: `user` | `disabled`; `style_policy`: `user` | `scene_auto` | `disabled`). The unified routing matrix dynamically resolves visual context from the selected stable or experimental preset and connected inputs.
   ...
   - *Subject + Scene Multi-Reference Routing*: When both Subject and Scene are connected, the free-target family keeps output geometry Scene-driven while target content remains empty. The identity-anchored family shifts both target content and geometry to the Subject image. `preserve_scene` explicitly anchors both target content and geometry to the Scene image. `scene_reinterpretation` uses the Scene for geometry and instruction guidance without initializing target latent from Scene pixels. `identity_transfer` uses Scene target content and geometry with Scene/Subject boosts of `2.0`; `subject_transfer_1` and `_2` remain empty-target test candidates with Scene geometry and Subject boosts of `5.0` and `6.0`.
   - *Automatic Style Policy (`scene_auto`)*: Applied to `identity_transfer`, the Subject Transfer candidates, and `scene_reinterpretation`. Automatically assigns the Scene reference as the style source when Scene is connected. Restored `transfer_identity_test_5` disables Style and Outfit.
