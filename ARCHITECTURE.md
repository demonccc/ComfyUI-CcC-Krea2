...
1. **Easy Routing Engine**: Subject, Scene, Outfit, and Style sources are managed via a policy-based capability matrix (`outfit_policy`: `user` | `disabled` | `automatic subject`; `style_policy`: `user` | `scene_auto` | `disabled`). The unified routing matrix dynamically resolves visual context from the selected stable preset and connected inputs.
   - *Subject-Aware Scene Geometry*: For Subject + Scene, Scene is first normalized to a 2.5 MP hard cap. If Subject fits, those dimensions are used unchanged. If not, the target grows with Scene aspect ratio just enough to contain Subject. Growth is capped at 2.5 MP, and only then may `contain_no_upscale` reduce Subject by the limiting width or height.
   - *Easy Aspect Ratio Canvas*: `auto` retains source-driven geometry. An explicit ratio builds the smallest `/16` canvas that contains Subject without interpolation, or Scene when Subject is absent. Subject takes precedence over Scene as the explicit-ratio anchor. Target content is placed with `contain_no_upscale`; the anchor is reduced proportionally only when the 2.5 MP hard cap makes preservation impossible.
