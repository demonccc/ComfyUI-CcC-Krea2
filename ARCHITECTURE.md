...
1. **Easy Routing Engine**: Subject, Scene, and Outfit logical roles are generated by the preset recipe. The unified routing matrix dynamically resolves the visual context based on the selected preset (e.g., `balanced`, `style_transfer`, `preserve_identity`, `outfit_transfer`) and the connected image inputs. This matrix automatically builds the internal reference chain, dynamically constructs semantic Scene+Outfit instructions, and filters disconnected references.
